...
func TestCertificateManagerAuthzFailClosed(t *testing.T) {
	dir := t.TempDir()
	key, err := store.LoadOrCreateMasterKey(filepath.Join(dir, "master.key"))
	if err != nil {
		t.Fatal(err)
	...
	mux := http.NewServeMux()
	(&certificatemanager.Service{Store: st, Authz: &authz.Evaluator{Policies: st}}).Mount(mux, func(*http.Request) (authn.Principal, bool) {
		return authn.Principal{Email: "nobody@example.com", IsRoot: false}, true
	})
	req := httptest.NewRequest(http.MethodGet, "/v1/projects/noctaxris-gcp-local/locations/global/certificates", nil)
	rec := httptest.NewRecorder()
	mux.ServeHTTP(rec, req)
