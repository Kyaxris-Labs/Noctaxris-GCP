...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td>Cloud Build</td>
      <td>createBuild nested step execution when the engine is configured (build SA <code>CLOUDSDK_AUTH_ACCESS_TOKEN</code>; ExtraHosts <code>host.docker.internal:host-gateway</code> when <code>NOCTAXRIS_GCP_INJECT_HOST_GATEWAY=1</code>, Compose default); named <code>serviceAccount</code> requires <code>iam.serviceAccounts.actAs</code>; missing engine stays WORKING (never SUCCESS); private pools require <code>cloudbuild.workerpools.use</code> on the host project; <code>NO_PUBLIC_EGRESS</code> via httpegress (WAN deny, in-emulator GCS and <code>host.docker.internal:4588</code> allow); shared regional triggers mux with Eventarc.</td>
