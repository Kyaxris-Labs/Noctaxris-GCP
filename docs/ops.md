...
3. Pull a Hub tag (`docker pull kyaxris/noctaxris-gcp:1.0.0`) or rebuild (`docker compose ... up --build`).
...
| Overlay | When |
|---------|------|
| `docker/compose.engine.yaml` | Opt-in restricted DinD (`noctaxris-gcp-engine`) for Cloud Run nested `:invoke` and shared `noctaxris-gcp-lab` SQL/Kafka/Redis. Sets `NOCTAXRIS_GCP_DOCKER_HOST` / `NOCTAXRIS_GCP_DOCKER_CERT_PATH`. No host publish of 2375/2376 or broker/DB ports. Never mounts host `docker.sock` |
