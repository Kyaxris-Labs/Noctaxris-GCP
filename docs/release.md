...
How to cut a public Noctaxris-GCP release (example: **1.2.0**). Docker Hub image: **`kyaxris/noctaxris-gcp`**.
...
1. Bump `VERSION` (plain text, e.g. `1.2.0`) and keep `internal/version/version.go` (and Dockerfile `ARG VERSION`) in sync.
...
git tag -a v1.2.0 -m "Release Noctaxris-GCP 1.2.0"
...
| Tag | Meaning |
|-----|---------|
| `kyaxris/noctaxris-gcp:1.2.0` | Exact semver |
| `kyaxris/noctaxris-gcp:1.2` | Major.minor |
...
```bash
docker build -f docker/Dockerfile --build-arg VERSION=1.2.0 -t kyaxris/noctaxris-gcp:local .
