...
| `NOCTAXRIS_GCP_INJECT_HOST_GATEWAY` | unset / false in Go; Compose `${NOCTAXRIS_GCP_INJECT_HOST_GATEWAY:-1}` | Set to `1` or `true` so nested Cloud Build steps get ExtraHosts `host.docker.internal:host-gateway` and gcloud/storage endpoint env pointing at `http://host.docker.internal:4588`. Code default is off (nil ExtraHosts in unit tests). Cloud Run nested one-shot stays `NetworkMode: none`. Overlay `compose.lab-host-gateway.yaml` pins `1`. |
| `NOCTAXRIS_GCP_HTTP_EGRESS` | empty (off) | Set to `1` to honor `NOCTAXRIS_GCP_HTTP_ALLOWLIST` for Pub/Sub push, Eventarc HTTP, Cloud Tasks, Scheduler, STS OIDC JWKS/discovery fetches, and Cloud Build private-pool step http(s) URLs beyond lab-local URLs. Unset/off: only `http://127.0.0.1:4588/_noctaxris-gcp/http-catcher...`, loopback `:4588` `/_noctaxris-gcp/oidc-lab/.well-known/...`, other loopback `:4588` lab-local URLs, and `http(s)://host.docker.internal:4588/...` (allowlist ignored). Port must be `4588`; other ports on `host.docker.internal` are denied. |
...
| `NOCTAXRIS_GCP_VPCSC_ENFORCE` | empty (off) | Set to `1` or `true` to deny cross-perimeter GCS upload/copy, Pub/Sub publish, Cloud KMS decrypt, and IAM Credentials `generateAccessToken` / `signBlob` / `signJwt` when a service perimeter restricts those APIs (dry-run `spec` included). STS is not perimeter-restricted. Default off keeps Access Context Manager CRUD theatre only. See [services/access-context-manager.md](services/access-context-manager.md). |
...
- Cloud Build ExtraHosts inject on by default (`NOCTAXRIS_GCP_INJECT_HOST_GATEWAY` `${...:-1}`). Go still defaults off.
