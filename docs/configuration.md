...
| `NOCTAXRIS_GCP_HTTP_EGRESS` | empty (off) | Set to `1` to honor `NOCTAXRIS_GCP_HTTP_ALLOWLIST` for Pub/Sub push, Eventarc HTTP, Cloud Tasks, Scheduler, STS OIDC JWKS/discovery fetches, and Cloud Build private-pool step http(s) URLs beyond lab-local URLs. Unset/off: only `http://127.0.0.1:4588/_noctaxris-gcp/http-catcher...`, loopback `:4588` `/_noctaxris-gcp/oidc-lab/.well-known/...`, and other loopback `:4588` lab-local URLs (allowlist ignored). |
